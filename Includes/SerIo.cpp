...
bool SerIo::Open()
{
	...
	if (m_portSettings->devicePath.find("pipe") != std::string::npos) {
		m_pipeHandle = CreateNamedPipeA(m_portSettings->devicePath.c_str(),
		                                PIPE_ACCESS_DUPLEX,
	...
	if (sp_open(m_portHandle, SP_MODE_READ_WRITE) != SP_OK) {
		spdlog::critical("SerIo::Init: Failed to open {}",
		                 m_portSettings->devicePath);
		return false;
	} else {
...
void SerIo::SendAck()
{
	constexpr static uint8_t ack = 0x06;
...
SerIo::Status SerIo::Write(std::vector<uint8_t>& buffer)
{
	...
	if (ret <= 0) {
		...
		spdlog::error("Only wrote {0:X} of {1:X} to the port!",
		              ret,
		              buffer.size());
		return Status::WriteError;
	}
...
SerIo::Status SerIo::Read(std::vector<uint8_t>& buffer)
{
	...
	if (m_isPipe) {
#ifdef _WIN32
		DWORD dwRet = 0;
		BOOL bRet   = ReadFile(m_pipeHandle,
                                     &serialBuffer[0],
                                     serialBuffer.size(),
                                     &dwRet,
                                     NULL);
		...
		ret = sp_nonblocking_read(m_portHandle,
		                          &serialBuffer[0],
		                          serialBuffer.size());
	}
